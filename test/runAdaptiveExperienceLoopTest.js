...
console.log('Compiling Adaptive Experience Loop test suite with tsc -p tsconfig.experience.json...');
try {
...
const compiledTest = path.join(testDistDir, 'test', 'adaptiveExperienceLoop.test.js');
console.log('Running compiled test suite at ' + compiledTest + '...\n');

try {
  const { runAdaptiveLoopTests } = require(compiledTest);
  runAdaptiveLoopTests()
    .then(() => {
