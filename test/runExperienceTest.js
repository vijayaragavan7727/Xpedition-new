...
console.log('Compiling Experience Engine test suite with tsc -p tsconfig.experience.json...');
try {
...
const compiledTest = path.join(testDistDir, 'test', 'experience.test.js');
console.log('Running compiled test suite at ' + compiledTest + '...\n');

try {
  const { runExperienceTests } = require(compiledTest);
  runExperienceTests()
    .then(() => {
