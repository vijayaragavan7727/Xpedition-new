...
console.log('Compiling Unified Experience Orchestrator test suite with tsc -p tsconfig.experience.json...');
try {
...
const compiledTest = path.join(testDistDir, 'test', 'orchestrator.test.js');
console.log('Running compiled test suite at ' + compiledTest + '...\n');

try {
  const { runOrchestratorTests } = require(compiledTest);
