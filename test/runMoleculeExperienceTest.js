...
console.log('Compiling 3D Molecule Builder test suite with tsc -p tsconfig.experience.json...');
try {
...
const compiledTest = path.join(testDistDir, 'test', 'moleculeExperience.test.js');
console.log('Running compiled test suite at ' + compiledTest + '...\n');

try {
  const { runMoleculeExperienceTests } = require(compiledTest);
  runMoleculeExperienceTests()
    .then(() => {
