...
console.log('Compiling 3D Object Manipulation test suite with tsc -p tsconfig.experience.json...');
try {
...
const compiledTest = path.join(testDistDir, 'test', 'objectExperience.test.js');
console.log('Running compiled test suite at ' + compiledTest + '...\n');

try {
  const { runObjectExperienceTests } = require(compiledTest);
  runObjectExperienceTests()
    .then(() => {
