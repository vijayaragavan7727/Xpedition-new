...
console.log('Compiling Programming Code Lab test suite with tsc -p tsconfig.experience.json...');
try {
...
const compiledTest = path.join(testDistDir, 'test', 'codeDebuggingExperience.test.js');
console.log('Running compiled test suite at ' + compiledTest + '...\n');

try {
  const { runCodeDebuggingExperienceTests } = require(compiledTest);
  runCodeDebuggingExperienceTests()
    .then(() => {
