        ...
        <main className="w-full my-auto grid grid-cols-1 lg:grid-cols-12 gap-y-2 sm:gap-y-5 lg:gap-y-4 lg:gap-x-10 xl:gap-x-14 items-center py-0.5 sm:py-6">
          ...
          <div className="order-1 lg:order-none lg:col-span-7 text-center lg:text-left">
            <div className="space-y-0.5 sm:space-y-3 mb-0 lg:mb-6">
              <h1 className="font-['Georgia',serif] text-[22px] sm:text-4xl md:text-5xl xl:text-[54px] font-bold text-slate-900 tracking-tight leading-[1.14]">
              ...
              <p className="font-sans text-[11px] sm:text-base font-medium text-slate-700 tracking-wide pt-0.5 sm:pt-1">
          ...
          <div className="order-2 lg:order-none lg:col-span-5 lg:col-start-8 lg:row-start-1 lg:row-span-3 flex justify-center lg:justify-end items-center w-full self-center">
            <Suspense
              fallback={
                <div className="w-full max-w-[340px] sm:max-w-[420px] h-[440px] bg-white rounded-[20px] animate-pulse shadow-xl border border-slate-100" />
              }
            >
              <LoginContent />
            </Suspense>
          ...
          <div className="order-3 lg:order-none lg:col-span-7 w-full max-w-sm lg:max-w-none mx-auto lg:mx-0">
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-1.5 sm:gap-3.5 lg:space-y-4 lg:gap-0 lg:mb-8">
              {valuePoints.map((item) => {
                const IconComponent = item.icon;
                return (
                  <div key={item.title} className="flex items-center gap-1.5 sm:gap-3.5 group text-left">
                    <div className="w-5 h-5 sm:w-8 sm:h-8 rounded-full bg-[#184E38] text-white flex items-center justify-center shrink-0 shadow-sm transition-transform duration-200 group-hover:scale-105">
                      <IconComponent className="w-2.5 h-2.5 sm:w-4 sm:h-4" aria-hidden="true" />
                    </div>
                    <span className="font-sans font-semibold text-slate-900 text-[11px] sm:text-[15px] leading-tight">
                      {item.title}
                    </span>
                  </div>
                );
          ...
          <div className="order-4 lg:order-none lg:col-span-7 text-center lg:text-left pt-0.5 lg:pt-2">
            <span className="font-caveat font-bold text-xl sm:text-3xl xl:text-4xl text-slate-900 tracking-tight leading-none inline-block -rotate-1 sm:-rotate-2 select-none">
